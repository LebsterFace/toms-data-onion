 ...
 */
fn read() -> String {
    let mut onion = fs::read_to_string("input.txt").unwrap();
...
fn split_to_chunks(input: String) -> (Vec<[u8; 5]>, usize) {
    ...
    let data = expanded
        ...
        .strip_suffix("~>")
        .unwrap();
    let (chunks, last): (&[[u8; 5]], &[u8]) = data.as_bytes().as_chunks::<5>();
...
fn ascii85(input: String) -> String{
    let (chunks, padding) = split_to_chunks(input);
    let decoded_chunks: Vec<String> = chunks.iter().map(|chunk| {
        ...
        String::from_utf8(large.to_be_bytes().to_vec()).unwrap()
    }).collect();

    let result = decoded_chunks.join("");
    return String::from(&result[..(result.len() - padding)]);
}

fn main() {
    let data = read();
    fs::write("./output.txt", ascii85(data));
}
